...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ----------------------------- CATALOGO -----------------------------
  // usa aquí tu catálogo completo tal como lo tienes (copia/pega)
  const catalogo = {
    insumos: [
      { clave: "S/C", descripcion: "BENZOCAÍNA 20% GEL, FRASCO 30 g", stock: "", minimo: "", caducidad: "" },
      { clave: "S/C", descripcion: "MEPIVACAÍNA 3% SIN VASOCONSTRICTOR. CAJA CON 50 CARTUCHOS DENTALES DE 1.8 ML CADA UNO.", stock: "", minimo: "", caducidad: "" },
  ...
  function moveButtonsToBottom() {
    ...
    if (!page2) return;
    // crear contenedor bottom si no existe
    let bottom = document.getElementById("controls-bottom");
    if (!bottom) {
      bottom = document.createElement("div");
      bottom.id = "controls-bottom";
      // estilos básicos (puedes ajustar en CSS externo)
      ...
      // insertar al final de page2
      page2.appendChild(bottom);
    }
    // mover botones existentes (si existen)
  ...
  function refreshDisabledOptions() {
    ...
    // recopilar valores seleccionados (no vacíos)
    const selectedValues = selects.map(s => s.value).filter(v => v && v !== "");
    // para cada select y cada option: si el option.value aparece en selectedValues pero no es el value del mismo select => disabled true
    selects.forEach(s => {
      Array.from(s.options).forEach(opt => {
        if (!opt.value) {
          // opción vacía no tocar
          opt.disabled = false;
          return;
        }
      ...
      // Si el select actual tiene una opción seleccionada que ha sido marcada disabled por lógica externa (edge-case),
      // permitimos que la opción seleccionada siga habilitada en su propio select:
      if (s.value) {
  ...
  function agregarFila() {
    ...
    // Input descripción: solo autocompleta en coincidencia exacta
    inputDesc.addEventListener("input", () => {
      const v = (inputDesc.value || "").trim();
      if (!v) {
        refreshDisabledOptions();
      ...
      // no sobrescribimos si no hay exacto
      refreshDisabledOptions();
      actualizarFila(tr);
    });

    // Enter/Tab intenta autocompletar si hay coincidencias claras
    inputDesc.addEventListener("keydown", (ev) => {
      if (ev.key === "Enter" || ev.key === "Tab") {
        const v = (inputDesc.value || "").trim();
        if (!v) return;
    ...
    // cada vez que hay cambios en la fila, refrescar disabled options
    ...
    // actualizar disabled inmediatamente (para que nuevas filas no permitan duplicados ya tomados)
    refreshDisabledOptions();
  }
  ...
  // Y cada vez que la ventana cambie tamaño o algo, volvemos a intentar mover (seguro)
