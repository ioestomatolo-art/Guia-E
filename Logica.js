// Logica.js — cliente (completo)
// Versión: elimina remoto + local, carga inventario y mantiene uid por fila.
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Sustituye con tu catálogo real; estructura: { categoria: [ {clave, descripcion, minimo, caducidad, stock}, ... ] }
  const catalogo = {
      insumos: [
  ...
  function agregarFila() {
    ...
    // botón eliminar: intenta borrar en servidor si fila tiene uid + hospital + categoría
    btnDel.addEventListener("click", async (ev) => {
      ...
      if (rowUid && hospitalKey && categoriaActiva) {
        try {
          ...
          // Preguntar al usuario si desea eliminar localmente a pesar del fallo remoto
          if (!confirm("No se pudo eliminar item en el servidor. ¿Eliminar localmente de todas formas?")) {
            btnDel.disabled = false;
            return;
          }
          // continúa y borra localmente
        } finally {
  ...
  function buildPayloadRows() {
    ...
    for (const row of tbody.rows) {
      ...
      row.dataset.uid = row.dataset.uid || filasExport[filasExport.length-1].uid;
    }
  ...
  // Eliminar filas seleccionadas (envía uids al servidor si existen)
